...
class NetworkMonitor(LoggerMixin):
    ...
    def generate_baseline(self):
        ...
        print(f"Базовая линия создана: {len(connections)} соединений записано.")
    ...
    def log_alert(self, alert_type, description, severity):
        ...
        # Сохранение в базу данных firewall, если доступна
        if self.external_db:
            try:
                self.external_db.insert_netsec_alert(alert_type, description, severity, json.dumps(alert))
            except Exception as e:
                logging.error(f"Failed to write alert to external database: {e}")
        else:
            # Fallback to console if no external DB
    ...
    def run_comprehensive_scan(self):
        ...
        rootkit_check = self.check_rootkit_network_hiding()
        if rootkit_check:
            print(f"ПРЕДУПРЕЖДЕНИЕ: Найдены несоответствия в данных соединений!")
        ...
        if docker_info and not any('error' in item for item in docker_info):
            print(f"Найдено {len(docker_info)} записей контейнеров.")
        else:
        ...
        if namespaces:
            ...
            print("Дополнительные сетевые пространства имен не найдены.")
        ...
        if sys.platform.startswith('linux') and os.geteuid() == 0:
            ...
            if packet_analysis and not any('error' in item for item in packet_analysis):
                print(f"Проанализировано {len(packet_analysis)} пакетов.")
        else:
        ...
        if self.alerts:
            ...
            if high_severity:
                print(f"Критические угрозы: {len(high_severity)}")
        else:
            print("\nСерьезных угроз не найдено.")
    ...
    def print_filtered_connections(self, connections, show_details=False):
        ...
        if not connections:
            print("Соединений с указанными фильтрами не найдено.")
            return

        print(f"\n{'=' * 80}")
        print(f"НАЙДЕННЫЕ СОЕДИНЕНИЯ: {len(connections)}")
        print(f"{'=' * 80}")
    ...
    def run_filtered_scan(self, filters=None):
        ...
        print(f"\nВсего соединений: {all_connections_count}")
...
def main():
    ...
    if sys.platform.startswith('linux'):
        if os.geteuid() != 0:
            ...
            print("Некоторые функции, такие как глубокий анализ пакетов, будут недоступны.")
            print("Для полного анализа запустите с: sudo python3 network_monitor.py")
            print()
    ...
    if args.baseline:
        ...
        if args.verbose:
            ...
            print("Для создания базовой линии используйте: --baseline")
            print("Для справки используйте: --help или --detailed-help")
        ...
        for pattern_type, items in patterns.items():
            if items:
                ...
                if len(items) > 5 and not args.verbose:
                    print(f"  ... и еще {len(items) - 5} элементов (используйте --verbose для подробностей)")
        ...
        if args.export:
            print(f"Экспорт результатов в формат {args.export}...")
...
def handle_filter_arguments(args, monitor):
    """Обработiа аргументов фильтрации"""
    filters = {}
